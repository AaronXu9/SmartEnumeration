...
using namespace std;
void read_sdf_frag(string filename, float**& x_l, float**& y_l, float**& z_l, char**& elm_l, char**& lab_l, int*& i_res_l)
{
	string line;
...
void read_sdf_protein(string filename, float**& x_p, float**& y_p, float**& z_p, char**& elm_p, char**& lab_p, int*& i_res_p)
{
	...
	if (myfile.is_open())
	{
		...
		while (!myfile.eof())
		{
			...
			getline(myfile, line);
			i_res_p[groupId] = stoi(line.substr(0, 4));

			x_p[groupId] = new float[i_res_p[groupId]];
			y_p[groupId] = new float[i_res_p[groupId]];
			z_p[groupId] = new float[i_res_p[groupId]];
			elm_p[groupId] = new char[i_res_p[groupId]];
			lab_p[groupId] = new char[i_res_p[groupId]];

			for (i = 0; i < i_res_p[groupId]; i++)
			{
				getline(myfile, line);
				x_p[groupId][i] = stof(line.substr(0, 10));
				y_p[groupId][i] = stof(line.substr(10, 10));
				z_p[groupId][i] = stof(line.substr(20, 10));
				elm_p[groupId][i] = line[31];
				lab_p[groupId][i] = line[35];
			}
			groupId++;
			i_count_p = groupId + 1;
			//break;
...
/*
bool writeout(std::string input, std::string output, std::string delimiter)
{
	//std::string input = "frgs_30000_AT2.sdf";
...
int main()
{
	...
	//read_sdf_frag("frags_600cpla2.sdf", x_l, y_l, z_l, elm_l, lab_l, i_res_l);
	//read_sdf_frag("frags_CB2.sdf", x_l, y_l, z_l, elm_l, lab_l, i_res_l);
	read_sdf_frag("fragments.sdf", x_l, y_l, z_l, elm_l, lab_l, i_res_l);
	//read_sdf_frag("f.sdf", x_l, y_l, z_l, elm_l, lab_l, i_res_l);
	read_sdf_protein("protein.sdf", x_p, y_p, z_p, elm_p, lab_p, i_res_p);
	//read_sdf_frag("frags_GHSR.sdf", x_l, y_l, z_l, elm_l, lab_l, i_res_l);
	//read_sdf_protein("GHSR.sdf", x_p, y_p, z_p, elm_p, lab_p, i_res_p);
	printf("\nN of FRAGMENTS         i_count_l= %4d", i_count_l);
	printf("\nN of PROTEIN ATOMS       i_res_p= %4d", i_res_p[0]);
	...
	for (i = 0; i < i_count_l; i++)	//ligand quantity___________________________________
	{
		...
		//printf("\nCAP COORDINATES             frag= %4d    max_label= %4d    xl= %4.4f    yl= %4.4f    zl= %4.4f", i, max_label[i], xll[i][1], yll[i][1], zll[i][1]);
		//_getch();
		if ((num_lab_l[i] == 5) || (num_lab_l[i] == 1))
		{
			max_label[i] = 2;
			num_current_1 = 1;
		}
	...
	//uuu:
		...
		if (num_lab_l[i] == 2)
		{
			...
			//pp_label2:
			for (i2 = second_l; i2 < i_res_l[i]; i2++)	//ligand quantity
			{
				if (lab_l[i][i2] == 49)
				{
					num_current_1 = 2;
		...
		//label_2:
			//printf("\nCAP COORDINATES             frag= %4d    max_label= %4d    xl= %4.4f    yl= %4.4f    zl= %4.4f", i, max_label[i],x_in, y_in, z_in);
			//_getch();
