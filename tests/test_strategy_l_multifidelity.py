"""Tests for Strategy L — multi-fidelity AL (single-shot UCB)."""
...
class TestStrategyL(unittest.TestCase):

    def test_single_shot_runs(self):
        from al_benchmark_gpr91.strategy_l_multifidelity import (
        ...
        self.assertEqual(result.extras["policy"], "l_mf")
        self.assertEqual(result.extras["n_probe"], 100)
    ...
    def test_prefers_better_mels_via_learned_surrogate(self):
        from al_benchmark_gpr91.strategy_l_multifidelity import (
            strategy_l_multifidelity_al,
        )
        syn, mel_ranked = _synthetic_universe()
